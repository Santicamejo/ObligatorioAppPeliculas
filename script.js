...
const ESTADISTICASPELICULAS = document.querySelector(
  "#pantalla-estdisticasPeliculas",
);
...
const BTNESTADISTICASPELICULAS = document.querySelector(
  "#btnMenuEstadisticasPeliculas",
);
...
function Inicio() {
  ...
  document
    .querySelector("#filtroFecha")
    .addEventListener("ionChange", aplicarFiltroFecha);
  ...
  document
    .querySelector("#btnRegistrarPelicula")
    .addEventListener("click", previaRegistrarPelicula);
...
function cargarPaises(listaPaises) {
  ...
  for (let unP of listaPaises) {
    miSelect += `<ion-select-option value="${unP.id}">
                ${unP.nombre}
            </ion-select-option>`;
  }
...
function registrarUsuario(nuevoUsuario) {
  fetch(`${URLBASE}/usuarios`, {
    ...
    .then(function (data) {
      if (data.codigo == 200) {
        const usuarioConectado = new UsuarioConectado(
...
function cargarCategorias() {
  ...
  for (let unaC of CATEGORIAS) {
    miSelect += `<ion-select-option value="${unaC.id}">
...
function cargarPeliculas(listaPeliculas) {
  ...
  if (!listaPeliculas || listaPeliculas.length === 0) {
    contenedor.innerHTML = `<ion-item lines="none">
  ...
  for (let unaP of listaPeliculas) {
    ...
    contenido += `<ion-item>
                <ion-label>
                    <h2>${unaP.nombre}</h2> <p>Categoría: ${categoriaTexto}</p> <p>Fecha de estreno: ${unaP.fechaEstreno}</p>
                </ion-label>
...
function cargarEstadisticas() {
  ...
  for (let unaC of CATEGORIAS) {
    ...
    contenido += `<ion-item>
                ...
                <ion-lable slot="end" color="primary">
                    ${contador}
                </ion-lable>
            </ion-item>`;
  }
  ...
  let porcentaje = total > 0 ? Math.round((mayores12 * 100) / total) : 0;
...
function mostrarMarkers(listaPaises) {
  for (let unPais of PAISES_MAPA) {
    ...
    L.circleMarker([unPais.lat, unPais.lng], {
      ...
      .addTo(map)
      .bindTooltip(
        `<strong>${unPais.nombre}</strong><br>${cantidad} usuarios`,
        {
          permanent: true,
          direction: "top",
          offset: [0, -5],
